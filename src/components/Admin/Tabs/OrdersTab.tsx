    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
           ...
           <p className="text-gray-500 text-[9px] sm:text-[11px] font-bold uppercase tracking-widest mt-1">Operasyonel Akış ve Lojistik</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="px-2 sm:px-3 py-1.5 rounded-lg bg-emerald-50 border border-emerald-100 flex flex-col items-end shrink-0">
            ...
            <span className="text-sm sm:text-lg font-bold text-emerald-700 leading-tight">{orders.length}</span>
          </div>
          <div className="px-2 sm:px-3 py-1.5 rounded-lg bg-gray-50 border border-gray-100 flex flex-col items-end shrink-0">
            <span className="text-[8px] sm:text-[10px] font-bold text-gray-500 uppercase tracking-tighter">Aktif</span>
            <span className="text-sm sm:text-lg font-bold text-gray-700 leading-tight">{orders.filter(o => o.status !== 'delivered' && o.status !== 'cancelled').length}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden flex flex-col text-[13px] shadow-sm min-h-[500px]">
        ...
        <div className="flex items-center border-b border-gray-200 overflow-x-auto hide-scrollbar bg-white">
          <button 
            ...
            className={`flex items-center gap-2 px-3 sm:px-4 py-3 min-w-max cursor-pointer transition-all border-b-2 ${
          ...
          >
            ...
            <span className="font-bold uppercase tracking-tight text-[10px] sm:text-xs">Hepsi</span>
          </button>
          <button 
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className="font-bold uppercase tracking-tight text-[10px] sm:text-xs">Bekleyen</span>
          ...
          >
            ...
            <span className="font-bold uppercase tracking-tight text-[10px] sm:text-xs">Kargo</span>
          </button>
          <button 
          ...
          >
            ...
            <span className="font-bold uppercase tracking-tight text-[10px] sm:text-xs">Tamam</span>
          </button>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex items-center gap-2 sm:gap-4">
                  <div className="flex flex-col w-20 sm:w-24 shrink-0">
                    <span className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-tighter truncate ${statusInfo?.color || 'text-gray-500'}`}>
                      {statusInfo?.label || order.status}
                    </span>
                    <span className="font-mono text-[9px] sm:text-[11px] font-bold text-gray-400">#{order.id?.slice(0,6).toUpperCase()}</span>
                  </div>

                  <div className="flex flex-col flex-1 min-w-0">
                    <span className="font-bold text-gray-900 truncate text-[11px] sm:text-[13px]">{order.user || 'Misafir'}</span>
